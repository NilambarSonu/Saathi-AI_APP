...
export async function getDashboardStats(): Promise<DashboardStats | null> {
  ...
  // If stats is null (e.g. 401) or doesn't have expected keys, return null so UI knows it's not a valid response
  if (!stats || (stats.farmsAnalyzed === undefined && stats.soilTests === undefined)) {
    return null;
  }
